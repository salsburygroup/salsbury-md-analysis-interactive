...
renderAnalysisTabs();renderOverview();renderFindings();renderModules();renderAccounting();renderVisuals();renderFigures();renderResources();renderQC();renderMolecules();const initialView=(location.hash||(DATA.reader_report_html?'#reader':'#overview')).slice(1);go(initialView.startsWith('reader-')?'reader':initialView);if(initialView.startsWith('reader-')){location.hash=initialView;document.getElementById(initialView)?.scrollIntoView()}
"""
