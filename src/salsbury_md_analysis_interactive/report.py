...
def _fes_visuals(report: Mapping[str, object]) -> List[Dict[str, object]]:
    ...
    if not isinstance(rows, list):
        ...
        primary_rows = [
            ...
            if row.get("smoothing_sigma_bins") == primary_sigma
        ]
        # The interactive state page presents one primary surface.  Alternative
        # smoothing levels remain available in the presentation sensitivity
        # table instead of multiplying nearly identical state panels.
        rows = primary_rows[:1] or candidates[:1]
...
_CSS = r"""
:root{--ink:#222222;--muted:#53565a;--paper:#f7f4ec;--card:#fff;--line:#d8d2c4;--wake-black:#000;--wake-gold:#9e7e38;--web-gold:#8c6d2c;--athletics-gold:#ceb888;--pale-gold:#f2eadb;--red:#9d2235;--pale-red:#f8e7ea;--shadow:0 10px 35px rgba(34,34,34,.09)}
*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.5 ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input,select{font:inherit}a{color:var(--web-gold)}
.shell{display:grid;grid-template-columns:270px minmax(0,1fr);min-height:100vh}.sidebar{background:var(--wake-black);color:#fff;padding:25px 18px;position:sticky;top:0;height:100vh;overflow:auto;border-right:5px solid var(--wake-gold)}.brand{font:700 22px/1.1 ui-serif,Georgia,serif;margin:0 8px 8px;color:var(--athletics-gold)}.subtitle{color:#e8ddc6;font-size:12px;margin:0 8px 25px}.nav button{display:block;width:100%;border:0;background:transparent;color:#f3efe6;text-align:left;padding:9px 12px;border-radius:10px;margin:2px 0;cursor:pointer}.nav button.active,.nav button:hover{background:var(--wake-gold);color:var(--wake-black)}.nav-heading{font-size:10px;text-transform:uppercase;letter-spacing:.12em;color:var(--athletics-gold);margin:18px 12px 6px}
.main{padding:32px clamp(20px,4vw,64px);max-width:1600px}.topline{display:flex;gap:15px;align-items:flex-start;justify-content:space-between;margin-bottom:22px}.topline h1{font:700 clamp(28px,4vw,46px)/1.05 ui-serif,Georgia,serif;margin:0}.eyebrow{text-transform:uppercase;letter-spacing:.12em;color:var(--web-gold);font-size:12px;font-weight:800}.status{border-radius:999px;padding:7px 12px;background:var(--pale-gold);color:var(--ink);border:1px solid var(--athletics-gold);font-weight:700;white-space:nowrap}.status.failed,.severity-error{background:var(--pale-red);color:var(--red)}.status.warning,.severity-warning{background:#fff3d6;color:#6b4e12}
...
.finding{display:grid;grid-template-columns:42px minmax(0,1fr);gap:12px;padding:15px 0;border-bottom:1px solid var(--line)}.finding:last-child{border:0}.rank{width:36px;height:36px;border-radius:50%;background:var(--pale-gold);color:var(--wake-black);display:grid;place-items:center;font-weight:800;border:1px solid var(--athletics-gold)}.badges{display:flex;gap:6px;flex-wrap:wrap;margin-top:7px}.badge{font-size:11px;border-radius:999px;padding:3px 8px;background:#eeeae2;color:#4a4640}.badge.sig{background:var(--pale-red);color:var(--red)}.finding-footer,.structure-links{display:flex;align-items:center;gap:9px;flex-wrap:wrap;color:var(--muted);font-size:11px;margin-top:8px}.inline-link{border:1px solid var(--athletics-gold);background:#fbf8f1;color:var(--web-gold);border-radius:7px;padding:4px 8px;cursor:pointer}.inline-link:hover{background:var(--pale-gold)}.filters{display:flex;gap:10px;flex-wrap:wrap;margin:12px 0 20px}.filters input,.filters select{border:1px solid var(--line);background:#fff;border-radius:9px;padding:9px 11px;min-width:180px}
.issue{padding:12px 14px;border-left:4px solid var(--line);background:#fafafa;margin:9px 0;border-radius:7px}.issue.error{border-color:var(--red)}.issue.warning{border-color:var(--wake-gold)}.issue code{font-size:11px}.module-row{border-bottom:1px solid var(--line);padding:12px 0}.module-row summary{cursor:pointer;font-weight:700;display:flex;justify-content:space-between;gap:12px}.metric-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:7px;margin:12px 0}.metric{background:#f6f2e9;padding:8px 10px;border-radius:7px;font-size:12px}.json{background:var(--wake-black);color:#f3efe6;padding:14px;border-radius:9px;overflow:auto;max-height:460px;font:12px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace}
...
function go(name,updateHash=true){$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${name}`));$$('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===name));document.body.dataset.view=name;if(updateHash)location.hash=name;window.scrollTo(0,0);if(name==='molecules'&&molecular.viewer)requestAnimationFrame(()=>{molecular.viewer.resize();molecular.viewer.render()})}
function routeHash(){const target=(location.hash||(DATA.reader_report_html?'#reader':'#overview')).slice(1);go(target.startsWith('reader-')?'reader':target,false);if(target.startsWith('reader-'))document.getElementById(target)?.scrollIntoView()}
...
function colorScale(t){t=Math.max(0,Math.min(1,t));const stops=[[247,244,236],[206,184,136],[158,126,56],[157,34,53],[45,8,15]];const p=t*(stops.length-1),i=Math.min(stops.length-2,Math.floor(p)),f=p-i;return `rgb(${stops[i].map((v,j)=>Math.round(v+(stops[i+1][j]-v)*f)).join(',')})`}
...
function drawStructurePreviews(host){$$('[data-structure-preview]',host).forEach(c=>{const s=DATA.structures.find(row=>row.structure_id===c.dataset.structurePreview);if(!s)return;const parsed=parsePDB(s.pdb_text),atoms=parsed.atoms.filter(a=>!waterResidues.has(a.res)),pts=atoms.map(a=>({a,x:.82*a.x+.38*a.z,y:.18*a.x+.86*a.y-.24*a.z}));if(!pts.length)return;const extent=Math.max(1,...pts.flatMap(p=>[Math.abs(p.x),Math.abs(p.y)])),scale=.42*Math.min(c.width,c.height)/extent,x=c.getContext('2d'),byIndex=new Map();x.fillStyle='#000';x.fillRect(0,0,c.width,c.height);pts.forEach(p=>byIndex.set(p.a.i,{...p,sx:c.width/2+p.x*scale,sy:c.height/2-p.y*scale}));const traces={};[...byIndex.values()].filter(p=>['CA','P'].includes(p.a.name)).forEach(p=>(traces[p.a.chain]??=[]).push(p));x.lineCap='round';x.lineJoin='round';Object.values(traces).forEach(t=>{let previous=null;t.forEach(p=>{if(!previous||Math.hypot(p.a.x-previous.a.x,p.a.y-previous.a.y,p.a.z-previous.a.z)>8){previous=p;return}x.beginPath();x.moveTo(previous.sx,previous.sy);x.lineTo(p.sx,p.sy);x.strokeStyle='#ceb888';x.lineWidth=6;x.stroke();previous=p})});parsed.bonds.forEach(([i,j])=>{const a=byIndex.get(i),b=byIndex.get(j);if(!a||!b||!isHetero(a.a)||!isHetero(b.a))return;x.beginPath();x.moveTo(a.sx,a.sy);x.lineTo(b.sx,b.sy);x.strokeStyle='#ddd4c2';x.lineWidth=2;x.stroke()});[...byIndex.values()].filter(p=>isHetero(p.a)).forEach(p=>{x.beginPath();x.arc(p.sx,p.sy,isIon(p.a)?7:2.8,0,Math.PI*2);x.fillStyle=elemColors[p.a.el]||'#d0a8c9';x.fill()})})}
...
function renderClusters(v,host){
  ...
  sizes.forEach((n,i)=>{const pct=100*n/total,bh=plotH*pct/100,x=left+i*bw+5,y=top+plotH-bh;svg+=`<rect x="${x}" y="${y}" width="${Math.max(4,bw-10)}" height="${bh}" fill="#9e7e38"><title>Cluster ${i+1}: ${n} frames (${pct.toFixed(1)}%)</title></rect><text x="${x+(bw-10)/2}" y="${top+plotH+18}" text-anchor="middle" font-size="11">${i+1}</text>`});
...
renderAnalysisTabs();renderOverview();renderFindings();renderModules();renderAccounting();renderVisuals();renderFigures();renderResources();renderQC();renderMolecules();window.addEventListener('hashchange',routeHash);routeHash();
"""
